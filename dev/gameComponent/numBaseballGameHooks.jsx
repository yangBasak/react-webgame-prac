...
function NumBaseballGame () {
  ...
  const [answer, setAnswer] = useState(()=>{getNumbers()}) //lazy init: 초깃값 설정때만 실행됨. 재렌더시 실행 x
  const [tries, setTries] = useState([])
  const onSubmitForm = (e) => {
    e.preventDefault();
    if (value === answer.join("")) {
      setResult("홈런")
      setTries((prevTries)=>{return [...prevTries, { try: value, result: "홈런" }]}) //이전 값 가져올때는 이런식으로
      alert("restart");
      setValue('')
      ...
      if (tries.length >= 9) {
        ...
        setTries((prevTries)=>{
          return [...prevTries, { try: value, result: `스트라이크:${strike}, 볼:${ball}` }]
        })
        setValue('')
      }
    }
